...
const displayFavorites= () => {
  ...
  if (data) {
    ...
    data.forEach( (fav) => {
      let btn = $("<button>");
      let text = document.createTextNode(formatLocation(fav["location"]));
      btn.append(text);
      btn.click( () => {
...
const displayFavoriteForecast= (location) => {
  let favorite_data = favorite(location);
...
const favorite= (location) => {
  let data = JSON.parse(sessionStorage.getItem("favorites"));
  return data.find( obj => formatLocation(obj.location) == formatLocation(location));
