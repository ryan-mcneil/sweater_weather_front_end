...
$(document).ready( () => {
  ...
  $('#add-favorite').click( () => addOrRemoveFavorite($('#add-favorite').value))
})
...
const displayFavorites= () => {
  ...
  if (data) {
    ...
    const favs = document.querySelector(".favorites-list");
    data.forEach( (fav) => {
...
const setFavoriteButton= (location) => {
  ...
  debugger
  if (favorite) {
    $('#add-favorite').text("Already a Favorite")
    $('#add-favorite').prop('disabled', true);
  } else {
    $('#add-favorite').text("Add to Favorites")
    $('#add-favorite').prop('disabled', false);
  }
}

const addOrRemoveFavorite= (location) => {
  const data = JSON.parse(sessionStorage.getItem("favorites"));
  const favorite = data.find( obj => obj.location === location );
